...
cd /data/picsl/mackey_group/public_data/ABCD/release2_site16_dicoms/

## now loop through the above array
for i in "${arr[@]}"
do
   echo "$i"
   cd /data/jux/mackey_group/public_data/ABCD/release2_site16_dicoms/sub-${i}
   python ${SCRIPTS_DIR}/3_fix_conflicting_study_identifiers.py
done
